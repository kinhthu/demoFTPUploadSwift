...
class VideoEditor {
    ...
    func editVideo(fromVideoAt videoURL: URL, forText text: String, speed: Double, onComplete: @escaping (URL?) -> Void) {
        //    print(videoURL)
...
//            try compositionTrack.insertTimeRange(timeRange, of: assetTrack, at: .zero)
            ...
            let duration = CMTimeMake(value: Int64(Double(asset.duration.value)*speed), timescale: asset.duration.timescale)
            compositionTrack.scaleTimeRange(timeRange, toDuration: duration)
            ...
            if let audioAssetTrack = asset.tracks(withMediaType: .audio).first,
               let compositionAudioTrack = composition.addMutableTrack(
                ...
                compositionAudioTrack.scaleTimeRange(timeRange, toDuration: duration)
            }
        } catch {
            print(error)
            onComplete(nil)
